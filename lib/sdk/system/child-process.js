        ...
        proc = subprocess.call({
          ...
          stderr: data => emit(child.stderr, 'data', data),
            ...
            if (!child.killed) {
              ...
              // node streams
              emit(child, 'exit', result.exitCode, null);
...
function execFile (file, ...args) {
  ...
  function exitHandler (code, signal) {
    ...
    console.log('myerror', error, code, signal);
    if (error)
    ...
    // Add the processes `killed` property to error for node parity
    if (error)
...
//    child.stdout.destroy();
//    child.stderr.destroy();
    console.log('killing with', options.killSignal, options);
    try {
  ...
  function pumpStdout (data) {
    stdout += data;
    console.log(stdout.length, options.maxBuffer);

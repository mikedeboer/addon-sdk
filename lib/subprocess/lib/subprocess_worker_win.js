...
function writePipe(pipe, data) {
    ...
    for (var chunk = 0; chunk <= numChunks; chunk ++) {
        ...
        for (var i=0; i < numBytes; i++) {
            var toWrite = data.charCodeAt(chunk * BufferSize + i) % 256;
            postMessage("attempting to write" + toWrite);
        ...
        //if (bytesWritten.value != numBytes)
            //throw(Object.keys(bytesWritten)+"error: wrote "+bytesWritten.value+" instead of "+numBytes+" bytes");
    }
